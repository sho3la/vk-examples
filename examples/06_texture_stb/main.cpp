...
private:
	...
	// Load the brick diffuse texture from disk using stbi_load, upload to GPU,
	// and create VkImageView + VkSampler.
	void createTexture()
	{
		// --- Load with stbi_load ---
		const std::string texPath = std::string(DATA_DIR) + "/brick/short_bricks_floor_diff_1k.jpg";
		int loadW, loadH, loadChannels;
		stbi_uc* pixels = stbi_load(texPath.c_str(), &loadW, &loadH, &loadChannels, STBI_rgb_alpha);
		if (!pixels)
		...
		createImage(static_cast<uint32_t>(loadW), static_cast<uint32_t>(loadH),
		...
		copyBufferToImage(staging, textureImage,
			static_cast<uint32_t>(loadW), static_cast<uint32_t>(loadH));
...
protected:
	...
	void onInitAfterCommandPool() override
	{
		...
		// Texture must be created before descriptor sets (needs imageView/sampler)
		createTexture();
